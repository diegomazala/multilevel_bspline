...
static std::function<decimal_t(decimal_t)> B[4] = {
    ...
    [](decimal_t t) { return (3 * std::pow(t, 3) - 6 * std::pow(t, 2) + 4) / 6; },
    [](decimal_t t) { return (-3 * std::pow(t, 3) + 3 * std::pow(t, 2) + 3 * t + 1) / 6; },
...
static std::function<decimal_t(decimal_t)> dB[4] = {
    ...
    [](decimal_t t) { return -(1.5 * t * t) + t + 0.5; }, [](decimal_t t) { return t * t / 2.0; }};
...
static std::function<decimal_t(decimal_t)> ddB[4] = {
    [](decimal_t t) { return -t + 1; }, [](decimal_t t) { return 3 * t - 2; },
    [](decimal_t t) { return -3 * t + 1; }, [](decimal_t t) { return t; }};
...
template <typename decimal_t>
static decimal_t tensor_dBB[3][3] = {
    {-1. / 12., -1. / 3., -1. / 12.}, {0., 0., 0.}, {1. / 12., 1. / 3., 1. / 12.}};

// The transpose of the matrix above, for the y-components,
// equivalent to B(k,s)*dB(l,t)
template <typename decimal_t>
static decimal_t tensor_BdB[3][3] = {
    {-1. / 12., 0., 1. / 12.}, {-1. / 3., 0., 1. / 3.}, {-1. / 12., 0., 1. / 12.}};
...
constexpr bool logically_equal(const T &a, const T &b, const T error_factor = 1)
{
    return a == b ||
           std::abs(a - b) <
               std::abs(std::min(a, b)) * std::numeric_limits<T>::epsilon() * error_factor;
}

template <typename decimal_t> class bspline_t
{
  public:
...
//protected:
    ...
    // Compute w_kl's and sum_sum w²_ab
    //
    static std::tuple<matrix4_t, decimal_t> compute_wkl_and_sum_w_ab2(decimal_t s, decimal_t t)
    {
        matrix4_t w;
